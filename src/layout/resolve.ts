 ...
 *  frame: header (1) + top rule (1) + bottom rule (1) + footer (1) = 4 rows
 *  (the horizontal rules ride with the header — they only show when the
 *  header is shown). We want the reader to retain ≥16 content rows — about
 *  a screenful of prose at typical paragraph density — so the threshold
 *  lands at 4 + 16 = 20. Below that the header (and rules) drop so short
 *  panes (tmux splits, etc.) keep the reader breathable.
 *
