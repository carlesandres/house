...
- Session-mutable options (CLI + env + file initial values, in-app `set`) are declared in `apps/house/src/config/options.ts` via `@house/options`. Add the spec there and thread layers through `loadConfig`; Browser holds a session for runtime changes. Do not add per-key parse helpers for catalog keys. Persist policy is `session` or `file`; file writes stay in `config/save.ts`. The package itself stays agnostic of TOML, Commander, and Effect ConfigProvider.
